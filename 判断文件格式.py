    ...
    with open(file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if re.findall('!\[(.*?)\]\((?:.*)\)', line):  # 因为文件名里可以有()，所以贪心匹配可能匹配到二位一体,所以下面用![分割
                ...
                for i in list1:
                    if re.findall('\]\((?:.*)\)', i):
                        temp = re.findall(re.compile(r'\]\((.*?)\.(png|jpg|jpeg|bmp|svg|tiff|webp|gif)(\s*)\)', re.S),
                                          i)
                        if temp != []:
                        ...
                        while True:             #重复的value要改掉,所以直接用了随机数
                            ...
                            if Random not in dict1.values():
                                dict1[link] = Random+os.path.splitext(link)[1] #随机数+扩展名
                                break
...
def downloadOrMoveImg(imgDir, dict1):
    ...
    for key, value in dict1.items():  # 需要下载图片
        if key.lower().startswith('http'):
            ...
            if os.path.normcase(os.path.split(os.path.abspath(key.strip('/').strip('\\')))[0])==os.path.normcase(imgPath): #图片不需要移动
                ...
                try:# 移动图片
                    ...
                    TempKey     = os.path.normcase(key)
                    newfile = os.path.normcase(newfile)
                    log += '\n[info] 移动 "%s" 到 "%s"'% (os.path.abspath(TempKey.strip('/').strip('\\')), newfile)
                    print('\033[1;32m[info]\033[0m 移动 "%s" 到 "%s"' % (os.path.abspath(TempKey.strip('/').strip('\\')), newfile))
                    if TempKey!=newfile:
...
def deleteMdAndImg(file,dict1):
    ...
    for i in dict1.keys():
        if not i.lower().startswith('http'):
            delfile=os.path.normcase(os.path.join(mdPath, i.strip('/').strip('\\')))
            if os.path.isfile(delfile):
                try:
                    ...
                    os.chmod(delfile, stat.S_IWRITE)  # 取消只读权限
                    os.remove(delfile)
            else:
...
if __name__ == '__main__':
    Logfile=os.path.join(os.getcwd(), time.strftime("%Y%m%d.log", time.localtime(time.time())))

    file = r'C:\Users\Abs1nThe\Desktop\DedeCMS v5.7 SP2_任意修改前台用户密码.md'
    imgDir='img'
    ...
    downloadOrMoveImg(imgDir, dict1)    #下载、移动图片
    updateMD(file, dict1,imgDir)        #更新md
