...
class Example(QMainWindow):
        ...
        with open(self.configFile, 'r', encoding='utf-8') as f:
            for line in f:
                if line.startswith('Log'):
                    ...
                    self.headers[line.partition('=')[0].strip()] = line.partition('=')[2].replace('\'', '"').strip()
    ...
    def saveConfig(self):
        ...
        with open(self.configFile, 'w', encoding='utf-8') as f:  # 写入新配置文件
            for key, value in {
                ...
                '': ['', '', '\n\n#用户header '+self.headerTxt1.text()+','+self.headerTxt2.text()+','+self.headerTxt3.text()+','+self.headerTxt4.text()+','+self.headerTxt5.text()+','+self.headerTxt6.text()],
    ...
    def menu(self):
        ...
        self.action_printHtml = QAction('响应html解码', self, checkable=True)
    ...
    def initUI(self):
        ...
        run1 = QPushButton("run", clicked=lambda: self.runReq(self.header1))
        run2 = QPushButton("run", clicked=lambda: self.runReq(self.header2))
        run3 = QPushButton("run", clicked=lambda: self.runReq(self.header3))
        run4 = QPushButton("run", clicked=lambda: self.runReq(self.header4))
        run5 = QPushButton("run", clicked=lambda: self.runReq(self.header5))
        run6 = QPushButton("run", clicked=lambda: self.runReq(self.header6))
        ...
        for i in range(len(self.listHeader)):
            if i == 6:
                break
            else:
                eval('self.headerTxt' + str(i+1) + '.setText(self.listHeader[' + str(i) + '].strip())')
        for i in ['self.proxy.setText(self.proxy1.strip())','self.onlyCookies.setText(self.onlyCookies1.strip())',
                  r'self.request.setText(ecd(self.request1.replace("\\n", "\n")))','self.delete_req_header.setText(ecd(self.delete_req_header1.strip()))',
    ...
    def runReq(self, textEdit):
        ...
        if not res:
            self.response.setText('<span style="color:#ff0000">request fail,check url</span>')
            self.status.showMessage('Error 请求失败,检查url', 5000)
            return
        ...
        ############## 更新token ##############
        if status==302 and 'Found' in body:
            tempheaders='Content-Type: application/x-www-form-urlencoded; charset=UTF-8'
            res1 = GetResponse('post', 'http://101.43.68.70:65150/login/verify', str_to_json(tempheaders), False, 'username=root&password=ABS%23npc.c', proxies)
            # # 从响应体获取
            ...
            textEdit.setColor()
            self.response.setText('<span style="color:#0000ff">已更新token</span>')
        ...
        if self.LogSwitch == 'True':
            ...
            with open(self.logFile, 'a', encoding='utf-8') as log:
                log.write(time.strftime("<details><summary>[%Y/%m/%d %H:%M:%S] ",
                                        time.localtime(time.time())) + url +
                          " ["+ecd(eval('self.headerTxt'+textEdit.objectName()[-1]).text())+"]</summary>" + # eval('self.headerTxt'+textEdit.objectName()[-1]).text() > self.headerTxt1.text()
        ...
        if len(lack_res_header) > 0:
            response_text += '<span style="color:#ff0000">缺失安全响应头: <br>' + '<br>'.join(
        ...
        if self.action_printHtml.isChecked():
            ...
            self.response.append(ecd(json_or_xml_body(res.headers, res.text.strip())))
