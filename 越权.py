...
class MyQTextEdit(QTextEdit):
    ...
    def setColor(self):
        if self.objectName()=='request': #self.request.setObjectName('request') 区分上色手法
            ...
            try:
                list1 = self.parent().parent().onlyCookies.text() #失焦触发
            except:
                list1 = self.parent().onlyCookies.text() #self.header1.setColor() 初次
            list1=list1.split(',')
            temp=[]
            for i in self.toPlainText().strip().split('\n'):
                if i:
                    key=ecd(i.partition(':')[0])
                    if key.lower() in [k.lower() for k in list1]:
...
class Example(QMainWindow):
        ...
        if not os.path.isfile(self.configFile):
            with open(self.configFile, 'w', encoding='utf-8') as f:
                f.write('#日志记录\nLog=True\n\n' +
                        ...
                        '#筛选请求头\nonlyCookies=Cookie,X-Csrf_Token\n\n' +
                        '#用户header\n' +
    ...
    #         f.writelines(temp)
    #         f.close()
    def saveConfig_windows(self):   # 创建新窗口
        ...
        grid1.addWidget(self.save_onlyCookies, 1, 1)
        grid1.addWidget(self.save_req, 2, 1)
        grid1.addWidget(self.save_req_header, 3, 1)
        grid1.addWidget(self.save_res_header, 4, 1)
        grid1.addWidget(self.selectAll_btn, 5, 1)
    ...
    def menu(self):
        ...
        action_SetCookie=QAction('Set-Cookie(开发中)', self, checkable=True)
    ...
    def initUI(self):
        ...
        grid.addWidget(QLabel('请求头1',self), 1, 0)
        grid.addWidget(QLabel('请求头2',self), 2, 0)
        grid.addWidget(QLabel('请求头3',self), 3, 0)
        grid.addWidget(QLabel('请求头4',self), 4, 0)
        grid.addWidget(QLabel('请求头5',self), 5, 0)
