...
class Example(QMainWindow):
    ...
    def runReq(self, textEdit):
        ...
        if req.strip() == '':
            self.response.setText('<span style="color:#DC143C">bad request</span>')
            return
        ...
        if res==False:
            self.response.setText('<span style="color:#DC143C">request fail,check url</span>')
        ...
        if self.LogSwitch == 'True':
            ...
            with open(self.logFile, 'a', encoding='utf-8') as log:
                log.write(time.strftime("<details><summary>[%Y/%m/%d %H:%M:%S] ",
                          ...
                          " ["+ecd(eval('self.headerTxt'+textEdit.objectName()[-1]).text())+"]</summary>" + # self.headerTxt1.text()
