...
def run(session):
    ...
    if proxy_button.isChecked():
        proxies.update({'http':proxy.text(),'https':proxy.text()})
    ...
    ############## 删除指定请求头 ##############
    if delete_header_button.isChecked():
    ...
    if check_res_header_button.isChecked():
        for key,value in str_to_json(check_res_header.toPlainText()).items():
            if key in res.headers.keys() and not(value==res.headers[key] or re.findall(value,res.headers[key])):
    ...
    if 'application/json' in res.headers['Content-Type'] and res.text[0]=='{':
        response.append(json.dumps(json.loads(res.text),indent=4,ensure_ascii=False,sort_keys=False,separators=(',',';')))
    elif 'xml'in res.headers['Content-Type'] and res.text[0]=='<':
        response.append(printXML(res.text).replace('ns0:',''))
    else:
        response.append('<br>'+ecd(res.text))
...
grid=QGridLayout()
https=QComboBox()
https.addItems(['https','http'])
grid.addWidget(https,0,0)
grid.addWidget(QLabel('session1'),1,0)
grid.addWidget(QLabel('session2'),2,0)
grid.addWidget(QLabel('session3'),3,0)
grid.addWidget(QLabel('session4'),4,0)
grid.addWidget(QLabel('session5'),5,0)
grid.addWidget(QLabel('session6'),6,0,2,1)
...
############## 3 ##############
proxy_button=QCheckBox("启用代理")
grid.addWidget(proxy_button,0,2)
run1=QPushButton("run")
run2=QPushButton("run")
run3=QPushButton("run")
run4=QPushButton("run")
run5=QPushButton("run")
run6=QPushButton("run")
run1.clicked.connect(lambda :run(session1.toPlainText()))
run2.clicked.connect(lambda :run(session2.toPlainText()))
run3.clicked.connect(lambda :run(session3.toPlainText()))
run4.clicked.connect(lambda :run(session4.toPlainText()))
run5.clicked.connect(lambda :run(session5.toPlainText()))
run6.clicked.connect(lambda :run(session6.toPlainText()))
...
############## 4 ##############
proxy=QLineEdit()
try:proxy.setText(proxy1.strip())
except:pass
requests1=QTextEdit()
requests1.setPlaceholderText("request")

delete_header_button=QCheckBox("删除指定请求头")
delete_req_header=QTextEdit()
try:delete_req_header.setText(delete_req_header1.strip())
except:pass

grid.addWidget(proxy,0,3)
grid.addWidget(requests1,1,3,5,1)
grid.addWidget(delete_header_button,6,3)
grid.addWidget(delete_req_header,7,3)

############## 5 ##############

response=QTextEdit()
response.setPlaceholderText("response")
check_res_header_button=QCheckBox("检查响应头")
check_res_header=QTextEdit()
try:check_res_header.setText(json_to_str(json.loads(check_res_header1)).strip())
except:pass

grid.addWidget(response,1,4,5,1)
grid.addWidget(check_res_header_button,6,4)
grid.addWidget(check_res_header,7,4)
...
check_res_header.setAcceptRichText(False)
response.setAcceptRichText(False)
delete_req_header.setAcceptRichText(False)
requests1.setAcceptRichText(False)
